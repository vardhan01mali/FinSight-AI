...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('finsight_token', response.data.access_token);
      localStorage.setItem('finsight_user_email', response.data.email);
      navigate('/');
    } catch (err: any) {
      console.error(err);
      setError(
    ...
    <div className="relative min-h-screen bg-slate-950 flex items-center justify-center px-4 overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 rounded-3xl p-8 shadow-2xl relative z-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-2">Sign in to your FinSight AI workspace</p>
        </div>
